...
public class ReparacionServiceTest {
    ...
    public void testCreateReparacion_withValidOrden() throws Exception {
        ...
        when(ordenService.getOrdenById(1L)).thenReturn(orden); // Mock expected behavior
        when(reparacionRepository.save(reparacion)).thenReturn(reparacion);
        ...
        verify(reparacionRepository, times(1)).save(reparacion); // Ensure repository save is called once
        verify(ordenService, times(1)).getOrdenById(1L); // Verify correct interaction with OrdenService
    }
    ...
    @Test
    public void testDeleteReparacion() {
        doNothing().when(reparacionRepository).deleteById(1L);

        assertDoesNotThrow(() -> reparacionService.deleteReparacion(1L));
        verify(reparacionRepository, times(1)).deleteById(1L);
    }
}
